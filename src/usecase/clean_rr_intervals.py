...
def compute_hrvanalysis_features(
    ...
    features_key_to_index: dict = FEATURES_KEY_TO_INDEX,
) -> str:
    """
    ...
    df_temp = pd.DataFrame(rr_intervals_without_outliers, columns=['rr_intervals'])
    df_temp['frame'] = sum_rrs
    df_temp['timestamp'] = df_temp['frame'].apply(
        lambda x: pd.Timedelta(x, unit='ms') + pd.Timestamp(start_timestamp)
    )
    ...
    df_temp["original_filename"] = rr_intervals_file_path
    ...
    df_temp = pd.DataFrame(interpolated_rr_intervals, columns=['rr_intervals'])
    sum_rrs = np.cumsum(interpolated_rr_intervals)
    df_temp['frame'] = sum_rrs
    df_temp['timestamp'] = df_temp['frame'].apply(
        lambda x: pd.Timedelta(x, unit='ms') + pd.Timestamp(start_timestamp)
    )
    ...
    df_temp = pd.DataFrame(interpolated_nn_intervals, columns=['rr_intervals'])
    sum_rrs = np.cumsum(interpolated_nn_intervals)
    df_temp['frame'] = sum_rrs
    df_temp['timestamp'] = df_temp['frame'].apply(
        lambda x: pd.Timedelta(x, unit='ms') + pd.Timestamp(start_timestamp)
    )
    ...
    df_temp = pd.DataFrame(wo_outliers_and_malik, columns=['rr_intervals'])
    df_temp['frame'] = sum_rrs
    df_temp['timestamp'] = df_temp['frame'].apply(
        lambda x: pd.Timedelta(x, unit='ms') + pd.Timestamp(start_timestamp)
    )
    ...
    # sum_clean_rrs = np.cumsum(clean_rrs)

    # df_temp = pd.DataFrame(clean_rrs, columns=['rr_intervals'])
    # df_temp['frame'] = sum_clean_rrs
    # df_temp['timestamp'] = df_temp['frame'].apply(
    #     lambda x: pd.Timedelta(x, unit='ms') + pd.Timestamp(start_timestamp)
    # )
    # pd.DataFrame(clean_rrs, columns=['rr_intervals'])
...
def parse_compute_hrvanalysis_features_args(
    ...
    parser.add_argument(
        "--rr-intervals-file-path", dest="rr_intervals_file_path", required=True
    )
    parser.add_argument("--output-folder", dest="output_folder")
    parser.add_argument("--sliding-window", dest="sliding_window", type=int)
    parser.add_argument("--short-window", dest="short_window", type=int)
    parser.add_argument("--medium-window", dest="medium_window", type=int)
    parser.add_argument("--large-window", dest="large_window", type=int)
