...
def write_quality_json(
    quality: List[int], exam_id: str, model_name: str, output_foder: str
) -> str:
...
def apply_ecg_qc(
    ...
    signal_subdiv = [
        signal[i * n : (i + 1) * n] for i in range((len(signal) + n - 1) // n)
    ]
    ...
    export_filepath = write_quality_json(
        signal_quality, output_folder, exam_id, model_name
    )
...
def parse_apply_ecg_qc_args(args_to_parse: List[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--exam-id", dest="exam_id", required=True)
    parser.add_argument("--output-folder", dest="output_folder")
    parser.add_argument("--sampling-frequency", dest="sampling_frequency")
    parser.add_argument("--model", dest="model", choices=MODELS)
