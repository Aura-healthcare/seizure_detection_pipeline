...
def ecg_channel_read(
    ...
    df_ecg = edfloader.convert_edf_to_dataframe(channel_name, start_time, end_time)

    sample_frequency = edfloader.sampling_frequency_hz

    annotation_file = os.path.join(data_path, patient, f"Annotations_EEG_{record}.csv")
    segments_file = os.path.join(data_path, patient, f"Segments_EEG_{record}.csv")

    df_annot = pd.read_csv(annotation_file, index_col=0, sep="|", encoding="latin-1")
