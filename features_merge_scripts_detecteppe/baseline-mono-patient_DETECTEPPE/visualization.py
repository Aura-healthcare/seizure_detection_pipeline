...
def _plot_umap(embedding, labels, patient_ids, results_dir, prefix, title_suffix='',
               pid_map=None, plot_patient_ids=True):
    """Plot UMAP embedding colored by patient_ids and by label."""
    os.makedirs(results_dir, exist_ok=True)

    # Patient ID plot (optionnel)
    if plot_patient_ids:
...
def _prediction_categories(true_labels, pred_labels):
    """Return 0=TN, 1=FP, 2=FN, 3=TP for each sample."""
    categories = np.zeros(len(true_labels), dtype=int)
    categories[(true_labels == 1) & (pred_labels == 1)] = 3
    categories[(true_labels == 0) & (pred_labels == 1)] = 1
    categories[(true_labels == 1) & (pred_labels == 0)] = 2
    return categories


def _plot_umap_errors(embedding, true_labels, pred_labels, results_dir, prefix, title_suffix=''):
    """UMAP coloré par catégorie TP/FP/FN/TN pour visualiser les erreurs du modèle."""
    os.makedirs(results_dir, exist_ok=True)

    categories = _prediction_categories(true_labels, pred_labels)

    style = {
        0: ('#aec6e8', 'o',  5,  0.25, 'TN — non-crise correct'),
        1: ('#ff7f0e', 'D', 45,  0.80, 'FP — non-crise prédit crise'),
        2: ('#d62728', 'x', 90,  1.00, 'FN — crise manquée ⚠'),
        3: ('#2ca02c', '*', 70,  0.90, 'TP — crise correct'),
    }

    fig, ax = plt.subplots(figsize=(10, 8))
    for cat in [0, 1, 3, 2]:   # FN plotté en dernier = premier plan
        mask = categories == cat
        if not mask.any():
            continue
        color, marker, size, alpha, lbl = style[cat]
        kwargs = dict(s=size, alpha=alpha, c=color,
                      label=f'{lbl}  (n={mask.sum()})', zorder=cat + 1)
        if marker == 'x':
            kwargs['linewidths'] = 2.2
        ax.scatter(embedding[mask, 0], embedding[mask, 1], marker=marker, **kwargs)

    ax.set_title(f'UMAP — erreurs de prédiction{title_suffix}', fontsize=14)
    ax.set_xlabel('UMAP 1')
    ax.set_ylabel('UMAP 2')
    ax.legend(markerscale=1.5, fontsize=10, loc='best')
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, f'{prefix}_errors.png'), dpi=300, bbox_inches='tight')
    plt.close()


def _plot_umap_errors_interactive(embedding, true_labels, pred_labels, results_dir, prefix,
                                  metadata=None, title_suffix=''):
    """Save an interactive HTML UMAP with hover metadata for prediction errors."""
    if metadata is None:
        return

    os.makedirs(results_dir, exist_ok=True)

    if len(metadata) != len(true_labels):
        logging.warning(
            "Interactive UMAP ignorée pour %s: metadata=%s, labels=%s",
            prefix, len(metadata), len(true_labels)
        )
        return
    ...
    style = {
        ...
        1: {
            ...
            "size": 10,
            "opacity": 0.85,
            "label": "FP — non-crise prédit crise",
        },
        2: {
            "color": "#d62728",
            "symbol": "x",
        ...
        3: {
            ...
            "size": 11,
            "opacity": 0.90,
            "label": "TP — crise correct",
        },
    }

    true_names = np.where(np.asarray(true_labels) == 1, "seizure", "no seizure")
    pred_names = np.where(np.asarray(pred_labels) == 1, "seizure", "no seizure")
    meta["true_label_name"] = true_names
    meta["pred_label_name"] = pred_names

    if "patient_id" not in meta.columns:
        meta["patient_id"] = ""
    if "seizure_id" not in meta.columns:
        meta["seizure_id"] = ""
    if "sample_id" not in meta.columns:
        meta["sample_id"] = np.arange(len(meta))

    patient_text = meta["patient_id"].fillna("").astype(str)
    seizure_text = meta["seizure_id"].fillna("").astype(str)
    seizure_text = seizure_text.where(seizure_text != "", "n/a")
    sample_text = meta["sample_id"].fillna("").astype(str)
    ...
    for cat in [0, 1, 3, 2]:
        ...
        hover_text = [
            (
                f"type: {cat_style['label']}<br>"
                f"patient_id: {patient}<br>"
                f"seizure_id: {seizure}<br>"
                f"true: {true_lbl}<br>"
                f"pred: {pred_lbl}<br>"
                f"sample_id: {sample}"
            )
            for patient, seizure, true_lbl, pred_lbl, sample in zip(
                patient_text[mask],
                seizure_text[mask],
                meta.loc[mask, "true_label_name"],
                meta.loc[mask, "pred_label_name"],
                sample_text[mask],
            )
        ]
        ...
        fig.add_trace(
            go.Scattergl(
                ...
                y=embedding[mask, 1],
                mode="markers",
                name=f"{cat_style['label']} (n={mask.sum()})",
                text=hover_text,
                hoverinfo="text",
                marker=dict(
                    color=cat_style["color"],
                    symbol=cat_style["symbol"],
                    size=cat_style["size"],
                    opacity=cat_style["opacity"],
    ...
    fig.update_layout(
        title=f"UMAP — erreurs de prédiction{title_suffix}",
        ...
        annotations=[
            dict(
                text="Survolez les points pour voir le patient et le seizure_id des fenêtres en erreur.",
                xref="paper",
                yref="paper",
                x=0,
                y=1.06,
    ...
    out_path = os.path.join(results_dir, f"{prefix}_errors_interactive.html")
...
def visualize_umap(encoded_data, labels, patient_ids, results_dir, prefix='umap',
                   ...
                   plot_patient_ids=True, test_preds=None, train_preds=None,
                   train_metadata=None, test_metadata=None):
    ...
    train_preds / test_preds: prédictions du modèle — si fournis, génèrent un plot d'erreurs TP/FP/FN/TN.
    train_metadata / test_metadata: DataFrame optionnel pour enrichir le hover HTML
    (patient_id, seizure_id, sample_id, ...).
    """
    os.makedirs(results_dir, exist_ok=True)
    ...
    if train_preds is not None:
        print("Creating train prediction error UMAP...")
        _plot_umap_errors(train_embedding, labels, train_preds,
                          results_dir, prefix, title_suffix=' (train)')
        _plot_umap_errors_interactive(
            train_embedding,
            labels,
            train_preds,
            results_dir,
    ...
    if test_data is not None:
        ...
        if test_preds is not None:
            print("Creating test prediction error UMAP...")
            _plot_umap_errors(test_embedding, test_labels, test_preds,
                              results_dir, f'{prefix}_test', title_suffix=' (test)')
            _plot_umap_errors_interactive(
                test_embedding,
                test_labels,
                test_preds,
...
def plot_evaluation_results(metrics, predictions_probs, true_labels, save_path='./evaluation_results.png'):
    ...
    all_labels = ['No Seizure', 'Seizure']
    tick_labels = all_labels[:cm.shape[0]]
    axes[1].set_xticklabels(tick_labels)
    axes[1].set_yticklabels(tick_labels, rotation=0)

    # Add metrics text
    n_non_seizure = int((true_labels == 0).sum())
    n_seizure = int((true_labels == 1).sum())
    metrics_text = (
        f"Accuracy: {metrics['accuracy']:.3f}\n"
        f"Precision: {metrics['precision']:.3f}\n"
        f"Recall: {metrics['recall']:.3f}\n"
        f"F1 Score: {metrics['f1']:.3f}\n"
        f"Samples — Non-seizure: {n_non_seizure}  |  Seizure: {n_seizure}  |  Total: {n_non_seizure + n_seizure}"
    )
...
def plot_cv_summary(summary, save_path='./cv_summary.png'):
    """Visualise les métriques par pli + mean ± std pour la cross-validation."""
    metrics = ['accuracy', 'recall', 'roc_auc', 'f1']
    labels  = ['Accuracy', 'Recall', 'ROC-AUC', 'F1']
    colors  = ['#4C72B0', '#DD8452', '#55A868', '#C44E52']

    folds     = [m['fold'] for m in summary['per_fold']]
    fold_labels = [f"Fold {f}" for f in folds]
    n_folds   = len(folds)
    x         = np.arange(n_folds)

    fig, axes = plt.subplots(1, len(metrics), figsize=(5 * len(metrics), 5))
    fig.suptitle('Cross-Validation Results', fontsize=15, fontweight='bold', y=1.02)

    for ax, metric, label, color in zip(axes, metrics, labels, colors):
        values   = [
            float(m[metric]) if m[metric] is not None and np.isfinite(m[metric]) else 0.0
        ...
        bars = ax.bar(x, values, color=color, alpha=0.75, width=0.55, zorder=2)
        ...
        if mean_val is not None and np.isfinite(mean_val):
            ax.axhline(mean_val, color='black', linewidth=1.5, linestyle='--', zorder=3,
                       label=f'Mean: {mean_val:.3f}')
            if std_val is not None and np.isfinite(std_val):
